...
| Red chest | the star rating of its vanilla item, ±1★ |

So a **five-star item** can only come from: a rank 5 or 5b blue chest, a rank 20
pot, barrel or cupboard, a monster drop at 1/256, or a red chest that already
held a four- or five-star item.

**Why red chests are the odd one out.** A blue chest, a pot and a drop all
carry a rank of their own, and the rank is what the randomizer reads. A red
chest carries no rank — its content is written into the zone script — so until
1.5 it accepted anything, and a late dungeon could hold a training vest. It
now keeps the star rating of the item it held in the original game, give or
take one star. `--coffres-rouges-libres`, and the launcher's *ignore where red
chests are*, restore the old behaviour; *Total chaos* ticks it.

**And a low-level item in a late dungeon is not always wrong.** The rank
belongs to the container, not to the zone: 227 zones mix pot ranks freely, and
`D13M02` alone runs from rank 2 to rank 20. A rank-3 pot in the last dungeon
gave a one-star item in the original game too, and it still does. The locations of
...
**Drawn at every battle (1.5, the "Monster drops" checkbox).** The item above is
fixed per monster when the ROM is built, so a given monster always drops the same
thing. With "Monster drops" checked, the chances stay the same but the item is
drawn **again at every battle**, from all 1,088 obtainable items, whatever the
monster. In the Balanced preset a 4- or 5-star item is kept only once in eight
draws (about 1 drop in 60 is 4-5★); in Total chaos every item has the same chance.
Note that in field battles the game takes the drop *chances* from the field
monster data, not from the table above: only the items come from it.
...
field) are documented with their evidence in [`../research/09-loot.md`](../research/09-loot.md),
...
## Consumables get a share

The game has 944 pieces of equipment against 144 consumables, so a plain
uniform draw makes nine containers out of ten give equipment -- which is what
1.2 and 1.3 did, and it got old fast.

The randomizer now gives consumables a share of the **weight** of each rank:
roughly 40 percent at the lowest ranks, 20 percent at the highest. Measured on
one seed: blue chests went from 11 to 32 percent consumables by weight, pots,
barrels and cupboards from 7 to 36 percent.

Nothing else moves. The odds that a container gives gold, an ambush or nothing
at all are the original game's, to the point. Every one of the 1,090 items is
still obtainable somewhere: the share is obtained by swapping which container
gets which item, never by dropping an item from the pool.

*ignore consumable share* in the application turns this off and gives you the
1.3 behaviour.
